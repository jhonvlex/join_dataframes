...
def find_fuzzy_matches(df1, df2, df1_colname, df2_colname, threshold):
    """
    Para cada name1:
    - Si encuentra un match con score >= threshold, lo guarda en 'matches'.
    - Si no hay ninguno que pase el threshold, guarda el mejor intento en 'non_matches'.
    """
    ...
    for name in lista_1:
        ...
        # Guardar según el umbral
        if best_score >= threshold:
            matches.append(resultado)
        else:
            non_matches.append(resultado)  # solo se guarda la mejor no-match si no pasó el threshold
...
def test(df, threshold):
    ...
    df = df.copy()
    df['match_pred'] = df.apply(lambda row: 1 if fuzz.ratio(row['name1'], row['name2']) >= threshold else 0,axis=1)
    return df
"""
